...
<div class="boughtDiv">
	<div class="orderType">
		<div class="selectedOrderType"><a orderStatus="all" href="#nowhere">所有申请</a></div>
		<!-- <div><a  orderStatus="waitPay" href="#nowhere">待定金</a></div> -->
		<div><a  orderStatus="waitDelivery" href="#nowhere">待审核</a></div>
			...
			<table class="orderListItemTable" orderStatus="${o.status}" oid="${o.id}">
				<tr class="orderListItemFirstTR">
					...
					<c:forEach items="${o.orderItems}" var="oi" varStatus="st">
							...
							<a href="foreproduct?pid=${oi.business.id}">${oi.business.name}</a>
						
					</c:forEach>
					<td>
								<c:if test="${o.status=='waitConfirm' }">
									<a href="foreorderConfirmed?oid=${o.id}">
										<button class="orderListItemConfirm">确认完成</button>
									</a>
								</c:if>
					...
					<tr class="orderItemProductInfoPartTR" >
						...
						<c:if test="${st.count==1}">
							...
							<td valign="top" rowspan="${fn:length(o.orderItems)}" width="120px" class="orderListItemProductRealPriceTD orderItemOrderInfoPartTD">
								<div class="orderListItemProductRealPrice">￥<fmt:formatNumber  minFractionDigits="2"  maxFractionDigits="2" type="number" value="500"/></div>
